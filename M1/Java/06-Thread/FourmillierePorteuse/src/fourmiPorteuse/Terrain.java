...
/**
 * Le terrain de jeu des fourmis. Il peut contenir au d̩part de 80x50 fourmis.
 * La dimension du terrain en X et en Y 
 * Une casier y sera repr̩sent̩e par un rectangle plein dont la dimension est
 * modifiable : (tailleFourmiX x tailleFourmiY).
 * 
 * @author maillot
 */
public class Terrain extends JPanel {
    private CaseDuTerrain[][] terrain;
    private int maxX = 80;
    private int maxY = 50;
    ...
    public Terrain(JLabel quantite) {
        ...
        terrain = new CaseDuTerrain[maxX][maxY];
    ...
    /**
     * Permet de tenir compte des redimensionnement de la fen̻tre.
     * Le calcul du nombre de fourmis en X et Y se fait en fonction
     * de la dimension de la fen̻tre et de la taille des fourmis en X et en Y.
     * 
     */
    private void changementDeDimension() {
        CaseDuTerrain[][] nq = new CaseDuTerrain[maxX][maxY];
    ...
    public void set(int x, int y, CaseDuTerrain casier) {
        try {
            terrain[x][y] = casier;
        } catch (ArrayIndexOutOfBoundsException aie) {
        }     
    }
    public CaseDuTerrain get(int x, int y) {
        try {
            return terrain[x][y];
