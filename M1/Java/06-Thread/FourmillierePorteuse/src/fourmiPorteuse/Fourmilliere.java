...
/**
 * JFrame qui possède <ol> <li>un label "quantite" au nord</li> <li>un bouton
 * "fourmi" au sud</li> <li>une zone de saisie "combien" au sud</li> <li>un
 * panneau dans lequel s'anime des fourmis au centre</li> </ol> Appuyer sur le
 * bouton "fourmi" lance le nombre de fourmis donnÃ© dans le champ de saisie
 * "combien" ou 1 fourmi si rien n'y est indiquÃ© ou si la champ n'est pas
 * convertible en entier. Le label "quantite" indique en permanance le nombre de
 * fourmis qui s'animent dans le panneau.
 *
 * @author maillot
 */
public class Fourmilliere extends JFrame {
    ...
    public Fourmilliere(String titre) {
        ...
        fourmi.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(java.awt.event.ActionEvent e) {
                ...
                try {
                    ...
                    terrain.ajouterFourmiNoire(terrain.getMaxX() / 2, terrain.getMaxY() / 2);
                    n = 0;
                }
                for (int i = 0; i < n; i++) {
                    terrain.ajouterFourmiNoire();
                }
                combien.setText("");
        ...
        tailleFourmi = new JSlider(JSlider.VERTICAL, 0, 100, 6);
                    ...
                    for (int y = 0; y < terrain.getMaxY(); y++) {
                        ...
                        if (casier != null) {
                            if (casier instanceof Fourmi) {
                                ((Fourmi) casier).setDuree(tailleFourmi.getValue());
                            }
                        }
        ...
        getJMenuBar().add(new JMenu("Ajouts"));
        
        JMenuItem miettes = new JMenuItem("Ajout de miettes");
        JMenuItem fourmis = new JMenuItem("Ajout de fourmis");
                
        getJMenuBar().getMenu(0).add(miettes);
        getJMenuBar().getMenu(0).add(fourmis);
        
        miettes.addActionListener(new ActionListener() {

            @Override
            public void actionPerformed(ActionEvent ae) {
                String combien = JOptionPane.showInputDialog("Combien ?");
                try {
                    int n = Integer.parseInt(combien);
                    for(int i = 0; i < n; i++) {
                        terrain.ajouterValise(random.nextInt(terrain.getMaxX()), random.nextInt(terrain.getMaxY()));
                    }
                    terrain.repaint();
                } catch(Exception ex) {
