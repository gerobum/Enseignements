...
// Les commentaires précédés de ###02 indiquent une modification à apporter
// pour réaliser la deuxième partie de l'exo : faire que la fourmi s'endorme au 
...
public class Fourmi extends Thread {
    ...
    private int x, y;
    // ###02 drapeau à rendre vrai pour stopper la fourmi.
    ...
    public Fourmi(Terrain terrain, int x, int y) {
        ...
        terrain.repaint();
        // ###02 Pour programmer l'endormissement de la fourmi. Une sorte de réveil
        // ##### à l'envers.
        new Thread() {
            @Override
            public void run() {
    ...
    public void run() {
        while (true) {
            try {
                avance();
                // ###02 La fourmi s'endort quand stop devient vrai
                while (stop) {
                    synchronized (this) {
                        wait();
                    }
