...
public class PanneauAnalyseurDeClasses extends JPanel {
  ...
  private final JTextField jtfNomClasse;
  ...
  private JComboBox<Class<?>> jcbDautresChoses;
  private Object valeur;
  private Class<?> type;
  private final FrameAnalyseurDeClasses parent;
  ...
  private final JPanel conteneurInstance = new JPanel();
  private final JPanel conteneurResultat = new JPanel();
  ...
  private void miseEnPlaceIHM() {
    ...
    jcbDautresChoses = new JComboBox<>();
    ...
    dansOuest.add(new JLabel("D'autres choses"));
    dansOuest.add(jcbDautresChoses);
  ...
  private void miseEnPlaceEcouteur() {
    jtfNomClasse.addActionListener(new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        // Cette méthode est lancée quand le nom d'une classe a été choisie.
        try {
          ...
          if (type == null) { 
            ...
            type = Class.forName(jtfNomClasse.getText());
          ...
          type = null;
          jtfNomClasse.setBackground(Color.yellow);
          jtfNomClasse.setForeground(Color.red);
        }
      }
    });
  ...
  private void remplirListes() {
    ...
    for (Constructor<?> c : type.getConstructors()) {
      jcbConstructeursPublics.addItem(c);
    }
    //??????????????????????????????????????????????????????????????????????????
    ...
    for (Method m : type.getMethods()) {
      jcbMethodesPubliques.addItem(m);
    }
    //??????????????????????????????????????????????????????????????????????????
    ...
    for (Method m : type.getDeclaredMethods()) {
      if (!Modifier.isPublic(m.getModifiers())) {
        jcbMethodesNonPubliques.addItem(m);
      }
    }
    //??????????????????????????????????????????????????????????????????????????
    jcbMethodesNonPubliques.addActionListener(selectionMethode);
    
    for(Class<?> ci : type.getClasses()) {
        jcbDautresChoses.addItem(ci);
    }
  }
