 ...
 */
package edu.uha.miage.mag.models.list;
...
public class PanierModel implements Serializable {
    ...
    public static Object handle(HttpServletRequest request) {
        ...
        if (request.getSession().getAttribute("modelList") == null) {
            model = new PanierModel();
            System.out.println("NULL");
        } else {
            model = (PanierModel) request.getSession().getAttribute("modelList");
            System.out.println("PAS NULL");
        }

        if (Articles.isAvailable(name)) {
            model.list.add(Articles.getArticle(name));
            model.total += Articles.getArticle(name).getPrice();
