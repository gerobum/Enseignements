...
<html>
    ...
    <body>
    <%--<c:if test="${!model.listEmpty}">
        <h1>Liste des rendez-vous</h1>
        
        <ol>
            <c:forEach items="${model.list}" var="i">
                <li>
                    ${i}
                </li>
            </c:forEach>
        ...
        <h1>Définition d'un rendez-vous</h1>
        <form method="post">
            <p>
                <label for="title">Titre : </label>
                <input type="text" id="title" name="title" value="${model.title}"/>
                <c:if test="${!model.titleOk}"><label for="title">Le titre ne doit pas être vide</label></c:if>
            </p>
            <p>
