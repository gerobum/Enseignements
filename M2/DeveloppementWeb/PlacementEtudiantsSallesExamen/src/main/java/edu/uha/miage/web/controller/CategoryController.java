...
// #### V1.0 Il sera renseigné dans le formulaire.
        model.addAttribute("category", etudiant);
...
// #### V1.0 /category/create (engendrée par le formulaire)
    @PostMapping("/create")
    public String created(@Valid Etudiant etudiant, BindingResult br) {
...
// #### V1.0 (HTTP/POST) réalise la modification s'il n'y a pas d'erreur.
    ...
    @PostMapping("/edit")
    public String edited(@Valid Etudiant etudiant, BindingResult br) {
        if (br.hasErrors()) {
            return "category/edit";
        }
